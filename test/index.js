...
describe("API ENDPOINT TESTING", () => {
    it('GET Landing Page', (done) => {
        chai.request(app).get('/api/v1/member/landing-page').end((err, res) => {
            ...
            expect(res.body.hero).to.have.all.key('travelers', 'treasures', 'cities')
    ...
    it('POST Detail Page', (done) => {
        const image = __dirname + '/buktibayar.jpeg';
        const sample = {
            image: image,
            idItem: "5e96cbe292b97300fc902223",
            duration: "2",
            bookingStartDate: "8-4-2022",
            bookingEndDate: "10-4-2022",
            firstName: "Bahar",
            lastName: "Suntar",
            email: "baharsuntar@gmail.com",
            phoneNumber: "08123123123",
            accountHolder: "suntar",
            bankFrom: "Mandiri",
        }

        chai.request(app).post('/api/v1/member/booking-page')
        .set('Content-Type', 'application.x-www-form-urlencode')
        .field('idItem', sample.idItem)
        .field('duration', sample.duration)
        .field('bookingStartDate', sample.bookingStartDate)
        .field('bookingEndDate', sample.bookingEndDate)
        .field('firstName', sample.firstName)
        .field('lastName', sample.lastName)
        .field('email', sample.email)
        .field('phoneNumber', sample.phoneNumber)
        .field('accountHolder', sample.accountHolder)
        .field('bankFrom', sample.bankFrom)
        .attach('image', fs.readFileSync(sample.image), "buktibayar.jpeg")
        .end((err, res) => {
            expect(err).to.be.null
            expect(res).to.have.status(200)
            expect(res.body).to.be.an('object')
